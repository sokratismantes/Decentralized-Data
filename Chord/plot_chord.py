...
def records_per_node_chord(ring):
    """
    Returns list of (node_id, record_count).
    Tries common storage access patterns.
    """
    out = []
    for n in getattr(ring, "nodes", []):
        cnt = 0
        if hasattr(n, "btree") and hasattr(n.btree, "get_all_items"):
            cnt = len(n.btree.get_all_items())
        elif hasattr(n, "data") and isinstance(n.data, dict):
            try:
                cnt = sum(len(v) for v in n.data.values())
            except Exception:
                cnt = len(n.data)
        elif hasattr(n, "storage") and isinstance(n.storage, dict):
            try:
                cnt = sum(len(v) for v in n.storage.values())
            except Exception:
                cnt = len(n.storage)

        out.append((getattr(n, "node_id", getattr(n, "id", None)), cnt))
    return out


def plot_main_chord_results(
    out_dir: Path,
    N0: int,
    insert_hops,
    update_hops,
    ...
    _hist(insert_hops, f"Chord INSERT hops distribution (N={N0})", "insert_hops_hist.png")
    _hist(lookup_hops, f"Chord LOOKUP hops distribution (N={N0})", "lookup_hops_hist.png")
    _hist(update_hops, f"Chord UPDATE hops distribution (N={N0})", "update_hops_hist.png")
    _hist(delete_hops, f"Chord DELETE hops distribution (N={N0})", "delete_hops_hist.png")
    ...
    _load_bar(load_after_join, "Load after 10 JOINS (records per node)", "load_after_joins.png")
    _load_bar(load_after_leave, "Load after 10 LEAVES (records per node)", "load_after_leaves.png")
    ...
    if join_moved_list and leave_moved_list:
        ...
        ax = fig.add_subplot(111)
        ax.boxplot(
            [join_moved_list, leave_moved_list],
            tick_labels=["JOIN moved", "LEAVE moved"],
            showfliers=False,
        )
    ...
    # join/leave overhead — total hops distributions
    if join_total_hops_list and leave_total_hops_list:
        fig = plt.figure(figsize=(9.2, 5.0))
        ax = fig.add_subplot(111)
        ax.boxplot(
            [join_total_hops_list, leave_total_hops_list],
            tick_labels=["JOIN total hops", "LEAVE total hops"],
            showfliers=False,
        )
        ax.set_title("Join/Leave cost (total hops) — distribution")
        ax.set_ylabel("Hops")
        ax.grid(True, axis="y", alpha=0.25)
        fig.tight_layout()
        fig.savefig(out_dir / "join_leave_total_hops_box.png", bbox_inches="tight")
        plt.close(fig)
